...
def match_vision_to_track(v_ego: float, lead: capnp._DynamicStructReader, tracks: dict[int, Track], path_x: list[float], path_y: list[float], current_prob_threshold: float, ignore_stationary: bool = False):
  ...
  # [修改] 將直線最高容許度從 1.0 降到 0.7，減少隧道兩側設施干擾
  ...
  # [修改] 解決遠距離機率倒掛問題。距離越遠，需要的視覺機率應越高 (50m 要 0.5，90m 提高到 0.6)
  ...
  if is_valid_lead:
    # [修改] 將上限提高到 60，提供更高的穩定鎖定緩衝
    track.is_stopped_car_count = min(track.is_stopped_car_count + 6, 60)
  ...
  if is_dynamic_target:
    ...
    # [修改] 靜止車輛必須穩定追蹤達到 50 分 (約 0.5 秒連續不丟失) 才會鎖定，避免閃影誤煞
    best_track = track 
