...
class ACM:
  ...
  def _apply_soft_hold(self, a_desired_trajectory, v_ego, lead, t_follow):
    ...
    if not should_cancel_soft_hold:
        ...
        # [雜訊過濾] 如果距離極近 (< 3m) 但視覺模型認為不是車 (< 30%)，判定為雷達物理雜訊，直接取消 Soft Hold
        if lead.dRel < 3.0 and lead_prob < 0.3:
            should_cancel_soft_hold = True
        
        # 判斷靜止時，要求視覺機率需 > 45% 才視為真車
        is_lead_stopped = (lead.vLead < 1.0) and (lead_prob > 0.45)

        # 速域動態判定：涵蓋全速域，包含 40 km/h 以上的無限制域 (acm 9 特色)
        if v_ego_kph <= 10.0:
    ...
    if should_cancel_soft_hold:
        ...
        if self.current_pitch <= PITCH_UPHILL_THRESHOLD:
            # 只要進入上限 0.99 以內且 TTC 危險即開始緩衝 (acm 9 移除下限的特色)
